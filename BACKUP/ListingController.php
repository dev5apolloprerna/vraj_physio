         ...
         try
        {
            if(auth()->guard('api')->user())
            {
                ...
                foreach ($schedule as $val) {
                    ...
                    if((optional($val)->total != $consumeAmount) || (optional($val)->total == 0 && $consumeAmount == 0)){
        ...
        {*/
            if(auth()->guard('api')->user())
            {
                 $User = auth()->guard('api')->user();
                    ...
                    $sessionMaster=SessionMaster::select('sessionmaster.*','patient_master.clinic_id',DB::raw("(select name from users where users.id=sessionmaster.therapist_id limit 1) as therpist_name")
                    ,DB::raw("(select treatment_name from treatment_master where treatment_master.treatment_id=sessionmaster.treatment_id limit 1) as treatment_name"))->where(['sessionmaster.patient_id'=>$request->patient_id,'patient_master.clinic_id'=>$request->clinic_id])
                        ...
                        ->join('patientordermaster', 'patientordermaster.patient_id', '=', 'sessionmaster.patient_id')
                        ->join('patientorderdetail', function ($join) {
                            $join->on('patientorderdetail.iOrderId', '=', 'patientordermaster.iOrderId')
                                 ->on('patientorderdetail.iTreatmentId', '=', 'sessionmaster.treatment_id'); // Changed to 'on' for proper JOIN condition
                        })->groupBy('sessionmaster.iSessionTakenId') // Ensure the column is correct
                 ...
                 $User = auth()->guard('api')->user();
                    ...
                    if($request->device_token != $User->device_token)
                    {
                     ...
                     $detail = Order::select(
                            ...
                            ->join('patientorderdetail', 'patientorderdetail.iOrderId', '=', 'patientordermaster.iOrderId')
                            ->where('patientordermaster.patient_id', $request->patient_id)
                            ->get();
                            ...
                            foreach ($detail as $val) {
                                ...
                                if (($val->total == $consumeAmount) && ($val->total != 0 && $consumeAmount != 0) ) 
                                {
                                    $scheduleList[] = [
        ...
        try
        {
            if(auth()->guard('api')->user())
            {
                 $User = auth()->guard('api')->user();
                    ...
                    $schedule=PatientSchedule::select('patient_schedule.*','patient_master.clinic_id','patientorderdetail.iAmount as total', 'patientorderdetail.iDueAmount as due','patientorderdetail.iPlanId','patientorderdetail.iSession','patientorderdetail.iOrderDetailId',DB::raw("(select name from users where users.id=patient_schedule.therapist_id limit 1) as therpist_name"),
                    DB::raw("(select treatment_name from treatment_master where treatment_master.treatment_id=patient_schedule.treatment_id limit 1) as treatment_name")
                  ...
                  WHERE patient_master.patient_id = patient_schedule.patient_id LIMIT 1) AS patient_name")
                    )->where(['patient_master.clinic_id'=>$request->clinic_id,'patient_schedule.day'=>$dayNumber])
                        ...
                        if(sizeof($schedule) != 0)
                        {
                            ...
                            foreach($schedule as $val)
                            {
                                ...
                                /*$detail = Order::select('patientorderdetail.iAmount as total', 'patientorderdetail.iDueAmount as due','patientorderdetail.iPlanId','patientorderdetail.iSession','patientorderdetail.iOrderDetailId')
                                            ->where([
                                                'patientordermaster.patient_id' => $val->patient_id,
                                                'iTreatmentId' => $val->treatment_id
                                            ])
                                ...
                                $therapistList=User::select('users.*','treatment_master.treatment_id','treatment_therapist.therapist_id','treatment_master.treatment_name')->where(['treatment_master.treatment_id'=>$val->treatment_id])
                                    ...
                                    ->whereDate('inDateTime', $date) // Use whereDate for date-only comparison
                                    ->first();
            ...
            try
         {
            ...
            if(auth()->guard('api')->user())
            {
                 $User = auth()->guard('api')->user();
                    ...
                    $schedule=SessionMaster::select('sessionmaster.*','patient_schedule.*','patient_master.clinic_id',DB::raw("(select name from users where users.id=sessionmaster.therapist_id limit 1) as therpist_name"),
                    ...
                    ,DB::raw("(SELECT CONCAT(patient_master.patient_first_name, ' ', patient_master.patient_last_name) 
                  FROM patient_master 
                  WHERE patient_master.patient_id = sessionmaster.patient_id LIMIT 1) AS patient_name")
                    )->where(['session_status'=>3,'patient_master.clinic_id'=>$request->clinic_id])
