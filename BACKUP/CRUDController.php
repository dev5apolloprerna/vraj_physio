        ...
        try
        {
            if(auth()->guard('api')->user())
            {
                 $User = auth()->guard('api')->user();
                    ...
                    $today=date('Y-m-d');
                    $inpatient=PatientIn::where(['patient_id'=>$request->patient_id,'therapist_id'=>$request->therapist_id,'treatment_id'=>$request->treatment_id])->whereDate('inDateTime', $today)->get();
                    $SessionMasterdata=SessionMaster::where(['patient_id'=>$request->patient_id,'therapist_id'=>$request->therapist_id,'treatment_id'=>$request->treatment_id,'created_at'=>$today])->first();

                    /*if(sizeof($inpatient) == 0)
                    {*/
                        if($request->isGroupSession == 1)
                        {
                            $GroupSession = 1;
                        }else{
                            $GroupSession=0;
                   ...
                   /* }else{
                        return response()->json([
                        'status' => 'error',
                        'message' => 'Patient already In for this Treatment',
                         ], 401);
                    }*/
