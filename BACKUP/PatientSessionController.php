         ...
         try 
        {
            if(auth()->guard('api')->user())
            {
                 $User = auth()->guard('api')->user();
                    ...
                    $schedule=PatientSchedule::where(['patient_id'=>$request->patient_id,'treatment_id'=>$request->treatment_id])->first();
                    ...
                    if ($session) 
                    {
	                    ...
	                    $ledger = PatientTreatmentLedger::where('patient_id', $session->patient_id)
							...
							$suggested=PatientSuggestedTreatment::where('patient_id', $session->patient_id)
							    ->where('treatment_id', $session->treatment_id)->first();
            ...
            if(auth()->guard('api')->user())
            {
                 $User = auth()->guard('api')->user();
                    ...
                    if($request->device_token != $User->device_token)
                    {
                        ...
                        $date=date('Y-m-d');
                        $session = PatientSchedule::where(['patient_schedule_id'=>$request->patient_schedule_id,'status'=>0,'leave'=>0])->first();
                        if ($session) 
                        {
                            if($request->status == 1)
                            {
                               ...
                               if($suggested)
                               {
                                 if ($suggested->iUsedSession == 0) 
                                    ...
                                    $suggested->iUsedSession=$used; 
                                    $suggested->iAvailableSession =$suggested->iSessionBuy - 1 ;
                                    $suggested->save();
                               }
                                    $session->leave=1;
                                    $session->status=1;
